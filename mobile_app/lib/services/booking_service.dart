...
class BookingService {
  ...
  /// Stands in for the real aggregator calling our webhook.
  /// Lets us test the full payment flow from the phone, with no real money involved.
  Future<void> simulateWebhook({required String providerReference, required String status}) async {
    ...
    if (response.statusCode != 200) {
      throw Exception(data['error'] ?? 'Failed to simulate payment result');
    }
  }

  Future<List<Booking>> fetchMyBookings() async {
    final response = await http.get(
      Uri.parse('${ApiService.baseUrl}/bookings/mine'),
      headers: await _authHeaders(),
    );
    final data = jsonDecode(response.body);
    if (response.statusCode != 200) {
