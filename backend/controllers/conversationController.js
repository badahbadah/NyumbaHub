const {
  findExistingConversation, createConversation, getConversationById, getConversationsForUser,
} = require('../models/conversationModel');
const { createMessage, getMessagesByConversationId, markMessagesAsRead, countUnreadMessagesForUser } = require('../models/messageModel');
...
exports.startOrSend = async (req, res) => {
  try {
    ...
    if (recipient_id === req.user.id) return res.status(400).json({ error: 'You cannot message yourself' });

    let conversation = await findExistingConversation(req.user.id, recipient_id, related_type, related_id);
    if (!conversation) {
      conversation = await createConversation({ initiator_id: req.user.id, recipient_id, related_type, related_id });
    }

    const message = await createMessage({ conversation_id: conversation.id, sender_id: req.user.id, message_text });
...
exports.getUnreadCount = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ error: 'Something went wrong while fetching unread count' });
  }
};